...
class Customizer extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if ( props.navbarType !== state.navbarType || props.footerType !== state.footerType || props.menuTheme !== state.menuTheme ||
      props.activeMode !== state.activeMode || props.sidebarState !== state.collapsedSidebar ) {
      return {
  ...
  handleNavbarChange = color => {
    ...
    this.setState({ activeNavbar: color })
  }

  componentDidMount() {
    this.setState({ navbarType: this.props.navbarType, footerType: this.props.footerType, menuTheme: this.props.menuTheme, activeMode: this.props.activeMode,
    collapseSidebar: this.props.sidebarState })
  }

  render() {
    const { activeNavbar, navbarType, footerType, menuTheme, activeMode, collapsedSidebar } = this.state
    return (
      <ContextLayout.Consumer>
        {context => {
          return (
            <div className={classnames("customizer d-none d-md-block", { open: this.props.customizerState === true })} >
              <div className="customizer-toggle" onClick={() => this.props.handleCustomizer(!this.props.customizerState) } >
              ...
              <div className="header d-flex justify-content-between px-2 pt-2">
                ...
                <div className="close-icon cursor-pointer" onClick={() => this.props.handleCustomizer(false)} >
                  <X size={24} />
                </div>
              </div>
              <hr />
              <PerfectScrollbar options={{ wheelPropagation: false }}  className="customizer-content p-2" >
                ...
                <div className="theme-mode">
                  ...
                  {/* <div className="d-inline-block mr-1">
                    <Radio
                      label="Light"
                      color="primary"
                      checked={activeMode === "light" || !["light", "dark", "green-dark"].includes(activeMode) ? true : false}
                      name="themeLayout"
                  ...
                  <div className="d-inline-block mr-1">
                    <Radio
                      label="green"
                      color="primary"
                      checked={activeMode === "green-dark" ? true : false}
                      name="themeLayout"
                      onChange={() => this.props.changeMode("green-dark")}
                    />
                  </div>
                  <div className="d-inline-block mr-1">
                    <Radio
                      label="pink"
                      color="primary"
                      checked={activeMode === "pink-dark" ? true : false}
                      name="themeLayout"
                      onChange={() => this.props.changeMode("pink-dark")}
                    />
                  </div>
                  <div className="d-inline-block mr-1">
                    <Radio
                      label="dark"
                      color="primary"
                      checked={activeMode === "real-dark" ? true : false}
                      name="themeLayout"
                      onChange={() => this.props.changeMode("real-dark")}
                    />
                  </div>
                  <div className="d-inline-block">
                    <Radio
                      label="blue"
                      color="primary"
                      checked={activeMode === "blue-dark" ? true : false}
                      name="themeLayout"
                      onChange={() => this.props.changeMode("blue-dark")}
                    />
                  </div>
                </div>
