...
const PaymentGateWayReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "PAYMENTGATEWAY_CASHFREE_CHEKOUT_DATA":
      return {...state, CashfreeCheckOutData: action.data}
    case "PAYMENTGATEWAY_RAZORPAY_CHEKOUT_DATA":
      return {...state, RazorpayCheckOutData: action.data}
    //---------------------
    case "TRANSACTION_HISTORY__GET_DATA":
      ...
      case "TRANSACTION_HISTORY__PAGENATION":
        return {
          ...state,
          data: action.data,
          totalPages: action.totalPages,
