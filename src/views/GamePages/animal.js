...
class Casino extends React.Component {
  ...
  componentDidMount(){
    this.props.data_load(providerconfig["ANIMAL"],"9","ANIMALSLIDERIMGS");
  ...
  typesActive = value => {
    this.props.gametypechange(value,providerconfig.ANIMAL)
  }

  provider_change = value =>{
      this.props.providerchange(value,providerconfig.ANIMAL)
  }
          ...
          <div style={{minHeight:"25rem"}}>                
            <Row className="casino-item-group">
              {indata.map((item,i) => (
                ...
                <CasinoItem data={item} me={this.props} />
                </Col>
              ))}
            </Row>
          </div>
...
const get_game  = (state) =>{
    ...
    return { 
      ...
      casinoSlider_images : state.auth.login.animal_images,
