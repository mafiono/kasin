...
import {Gamelist_star} from "../../configs/providerConfig"
...
export class bazaar extends Component {
    ...
    Event(bazaaritem,ids,timers,index,location){
        var gameitem = this.state.gamelist.find(obj=>obj._id === ids);
            ...
            let remain_t1 = this.get_remaining_time(bazaaritem.timers.closetime);
        ...
        return <React.Fragment>
            {
                ...
                gamesdata.slice(s,e).map((gameitem,j)=>(
                    <React.Fragment key={j}>
                        ...
                        {
                            item.gamelink && item.gamelink[gameitem._id] && item.gamelink[gameitem._id].status && item.timers.opentime && item.timers.closetime ? 
                                <>
                ...
                <Media queries={{ small: "(max-width: 768px)",large: "(min-width: 769px)" }}>
                    {matches => (
                        ...
                        <>
                             <Col md="12" >
                                {
                                    Gamelist_star.slice(0,3).map((item,i)=>(
                            ...
                            <Col md="12" className="mt-1">
                                {
                                    Gamelist_star.slice(3,6).map((item,i)=>(
                                        <div className="tblbazaarheader" key={i} style={{width :"33.3%"}} >
                                            {item}
                                        </div>
                                    ))
                                }
                        ...
                        <>
                            <Col md="12" className="">
                                {
                                    Gamelist_star.map((item,i)=>(
                                        <div className="tblbazaarheader" style={{width:"16.6%"}} key={i}>
                                            {item}
                                        </div>
                                    ))
                                }
                            </Col>
                                {
                                    ...
                                    <React.Fragment key={i}>
                                        <Col md="12" className="d-flex">
                                            <div className="tblbazaaritem" style={{width:"16.6%"}}>
                                                <div className="child d-block text-center justify-content-center">                                                
                                                    <span className="d-block"> 
                                                        { item.bazaarname }
                                                    </span>
                                            ...
                                            <div className="tblbazaaritem" style={{width:"16.6%"}}>
                                                ...
                                                {
                                                    this.games_part_render(item,16.6,0,4,i,"1")
                                                }
                                        </Col>
                                        <Col md="12">
                                            {
